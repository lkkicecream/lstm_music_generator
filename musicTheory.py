...
def makeChord(notes, quarterLengt=4, key=None, way='symmetrical-fifth'):
    '''
        依照給定的音符序列找出適當的和弦
        ...
        way: 巡迴的方式：可選擇上五度（'fifth'）、下五度（'forth'）、上半音（'halftone'）、下半音（'halftone-down'）、對稱五度（'symmetrical-fifth'，預設）
    '''
    # 計算該片段中每個音出現的次數
    ...
    # 判斷的順序，越前面優先權越高
    chordPatternSequrence = [major[:], minor[:], dim[:], arg[:], dom7[:], dim7[:], maj7[:]]
