...
def get_notes():
    ...
    for midi_file in glob.glob("music_midi/*.mid"):
        ...
        if parts:  # 如果有乐器部分，取第一个乐器部分
            ...
            print(parts.parts[0])
            notes_to_parse = parts.parts.recurse()  # 递归
    ...
    for data in prediction:
        ...
        if ('.' in data) or data.isdigit():  # data中有.或者有数字
            ...
            for current_note in note_in_chord:
                bass_note = note.Note(int(current_note))  # 把当前音符化成整数，在对应midi_number转换成note
                bass_note.storedInstrument = instrument.Harpsichord()  # 乐器用钢琴
        ...
        # 是note格式：
            ...
            bass_note = note.Note(data)  # note直接可以把data变成新的note
            bass_note.offset = offset
            bass_note.storedInstrument = instrument.Harpsichord()  # 乐器用钢琴
            output_notes.append(bass_note)  # 把new_note传到output_notes中
        # 每次迭代都将偏移增加，防止交叠覆盖
        offset += 0.5
